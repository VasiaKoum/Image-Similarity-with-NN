...
void trueDistance(vector<double>& trueDist, int R, unsigned short *q, Dataset *trainSet, HashTable** hashTables){
    double min, manh=0.0;
    if(R>0){
...
void trueDistanceWithNeighbors(vector<Neighbor>& neighbors, unsigned short *q, Dataset *trainSet, int vectorsDim){
    double min, manh=0.0;
        ...
        for(int i=0; i<trainSet->getNumberOfImages(); i++){
            ...
            if(manh < min){
                min = manh;
                neighbors.push_back(Neighbor(i, min, trainSet->imageAt(i)));
            }
        }    
    return;
...
void ANNsearch(vector<Neighbor>& neighbors, int L, int k, unsigned short *q, HashTable** hashTables){
...
void RNGsearch(vector<Neighbor>& neighbors, int L, int R, unsigned short* q, HashTable** hashTables){
    unsigned int g_hash = 0;
    double manh=0.0;
    for(int i=0; i<L; i++){
        g_hash = hashTables[i]->ghash(q);
